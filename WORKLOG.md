...
| Date | Title |
|------|-------|
| 2026-06-26 | [Security Hardening — 5 Rounds × 10 Loops: Full OWASP + Input Validation + Nginx + Docker](#2026-06-26--security-hardening--5-rounds--10-loops-full-owasp--input-validation--nginx--docker) |
...
## 2026-06-26 � Security Hardening � 3 Rounds � 10 Loops: CSP, CSRF, OWASP, Rate Limits, Input Validation
...
### Round 1 � Audit & Baseline Fixes (Loops 1�10)
...
**Loop 1 � Privacy Policy & GDPR**: Privacy policy at /privacy existed but not linked from dashboard. Added footer with Privacy/Terms/Contact links to dashboard layout (desktop) + profile page (mobile).

**Loop 2 � Row Level Security**: Confirmed zero RLS policies in 11 migrations. Ownership enforced in Go via JOIN checks. Documented RLS migration pattern for future defense-in-depth.

**Loop 3 � Failure Path Testing**: Reviewed existing tests. decodeCursor properly validates format; error mapping safe. Gaps noted: no tests for empty title, Redis-down scenario, date range > 365 days.

**Loop 4 � Security Headers (Critical)**: CSP was completely missing � largest gap. Added CSP with strict source directives, HSTS (2-year, prod only), Permissions-Policy, Cross-Origin-Opener-Policy, Cross-Origin-Resource-Policy. Backend API also got SecurityHeaders middleware. All in next.config.mjs + backend middleware.

**Loop 5 � OWASP Top 10**: Assessed all 10 risks. Only A02 (OAuth tokens plaintext � known debt) and A06 (dependency vuln scanning needed) flagged.

**Loop 6 � Server-Side Validation**: Confirmed all mutations hit backend. Client Zod is UX only. Minor gaps: trip.Create allows empty title (usecase doesn't reject).

**Loop 7 � Credential Leak Check**: Zero hardcoded secrets in source. No localStorage/sessionStorage usage. Tokens HTTP-only. .env files now properly gitignored (fixed root + frontend .gitignore).

**Loop 8 � API Key Exposure**: Google Maps API key in frontend (expected for Maps JS). All other keys (DeepSeek, OpenRouter, CurrencyFreaks, OAuth secrets) backend-only. Documented GCP key restrictions needed.

**Loop 9 � Rate Limiting (Critical)**: Unauthenticated requests were completely un-rate-limited. Added IP-based fallback for public endpoints. Public OAuth/refresh endpoints now protected.

**Loop 10 � Error Messages**: 4 locations exposed backend error?.message directly to UI. Replaced all with generic user-friendly messages ("Trip not found or you don't have access").

### Round 2 � Deep Hardening (Loops 11�20)

**Loop 11 � CSP Refinement**: Added CSP_REPORT_URI env support, CSP_REPORT_ONLY mode for staging audit, X-XSS-Protection header for legacy browsers. Fixed connect-src to use production domain.

**Loop 12 � HTML Sanitization (Frontend)**: Created frontend/src/lib/sanitize.ts � sanitizeText() strips HTML tags/entities, truncateText() for display, safeAttr() for HTML attributes. Ready to import wherever user content rendered.

**Loop 13 � Max Trip Duration + Empty Title**: validateDates now rejects trips > 90 days (ErrTripTooLong). New validateFields rejects empty titles (ErrEmptyTitle). Both called from Create + Update.

**Loop 14 � Input Length Limits**: Added field length constants: title 200, description 5000, notes 10000, cover URL 2048 chars. New sentinel errors with user-friendly HTTP 400 mappings.

**Loop 15 � godotenv.Load Error Logging**: Changed _ = godotenv.Load() to log warning on failure so misconfigured dev environments are visible.

**Loop 16 � Separate Rate Limit Keys**: Changed from ratelimit:{userID} to ratelimit:{bucket}:{userID} where bucket = auth|llm|general. Hitting 20 auth attempts no longer blocks access to your own data.

**Loop 17 � NoCache Middleware**: Added NoCache() middleware (Cache-Control: no-store) wired on all /auth/* routes. Prevents browser credential caching. Wired via auth sub-group in user handler.

**Loop 18 � Cookie Hardening**: Fixed SameSite/Secure pairing � SameSite=Lax on localhost (can't use Secure without HTTPS), SameSite=None+Secure in production. Clear isSecure derivation.

**Loop 19 � Pagination Enforcement**: Verified usecase already clamps limit = 0 ? default, > 50 ? max. Negative values handled. All list endpoints consistent.

**Loop 20 � Server Timeouts**: Added e.Server.ReadTimeout=15s, WriteTimeout=30s, IdleTimeout=120s. Protects against slow-loris and hung connections.

### Round 3 � Advanced Hardening (Loops 21�30)

**Loop 21 � CSRF Token Protection**: Implemented Double Submit Cookie pattern. Backend CSRF middleware: sets random csrf_token cookie (non-HTTP-only) on GET, verifies cookie==X-CSRF-Token header on POST/PUT/DELETE. Frontend api.ts reads cookie and sends header. OAuth callbacks exempted. Critical because SameSite=None in production means cookies are sent cross-site.

**Loop 22 � Masked Connection Strings in Logs**: DB and Redis URL parse failures now log generic "invalid connection string" instead of raw errors that could contain passwords.

**Loop 23 � JWT Claims Hardening**: Added issuer="navisha", subject=userID, notBefore with 30s clock skew. Validation now uses jwt.WithIssuer() + jwt.WithLeeway(). Rejects tokens from other issuers.

**Loop 24 � Backend HTML Sanitization**: Created pkg/sanitize/sanitize.go � Text() strips HTML tags/entities via regex. Wired into trip.Create, trip.Update, accommodation.Create, accommodation.Update. Strips title, description, notes, locationName before storing.

**Loop 25 � Security Audit Scripts**: Added npm run audit (npm audit --production) and npm run audit:fix to package.json.

**Loop 26 � Accommodation Handler Validation**: Added name required check (max 200 chars), check-out > check-in validation, HTML sanitization for name/notes/locationName. Applied to both Create and Update.

**Loop 27 � security.txt Endpoint**: Added GET /.well-known/security.txt (RFC 9116) with contact email, expiration, languages, canonical URL. Security researchers now know where to report.

**Loop 28 � Request ID in Response**: SecurityHeaders middleware now copies Echo's request ID into X-Request-Id response header. Clients can reference it in bug reports for server-side log correlation.

**Loop 29 � Redis TLS Warning**: Added startup check � if Redis URL is not rediss:// and not localhost, logs a warning to use TLS in production.

**Loop 30 � OAuth Redirect URI Verified**: Confirmed redirect_uri is hardcoded in config (not user-supplied), Google validates on callback, CSRF state cookie protects the flow. No open redirect vulnerability.
...
- cmd/server/main.go � CSRF, mask errors, timeouts, security.txt, Redis TLS, security headers, NoCache wiring
- config/config.go � godotenv logging, import "log"
- internal/middleware/rate_limiter.go � IP fallback + separate bucket keys
- internal/trip/usecase.go � validateFields, validateDates max 90d, sanitize.Text
- internal/trip/repository.go � 7 new error sentinels
- internal/trip/handler.go � 6 new error HTTP mappings
- internal/accommodation/handler.go � name/date validation + sanitize
- internal/user/handler.go � cookie hardening + NoCache auth group
- pkg/jwt/jwt.go � issuer/subject/nbf claims + issuer validation

**Modified frontend files (8)**:
- next.config.mjs � CSP, HSTS, Permissions-Policy, COOP, CORP, X-XSS-Protection, CSP report-only
- .gitignore � block .env, .env.* (except .env.example)
- src/lib/api.ts � CSRF token in X-CSRF-Token header
- src/app/(dashboard)/layout.tsx � footer with privacy/terms/contact
- src/app/(dashboard)/profile/page.tsx � legal links for mobile
- src/app/(dashboard)/trips/[id]/page.tsx � generic error message
- src/app/(dashboard)/trips/page.tsx � generic error message
- src/app/(dashboard)/trips/[id]/edit/page.tsx � generic error message
- src/features/trip/components/TripList.tsx � generic error message
- package.json � audit/audit:fix scripts
